...
function renderPlayerList() {
  ...
  const activeCount = Object.values(State.players).filter(
    (p) => p.role !== "zuschauer",
  ).length;
...
async function forceBackToLobby() {
  // Lobby direkt schließen – alle Clients fliegen raus
  if (!State.isHost) return;
  if (
    !confirm("Lobby schließen? Alle Spieler werden zurück zum Start gesetzt.")
  )
    return;

  log("Host schließt Lobby aus dem Spiel heraus");
  removeCentralListener();
  removeAllListeners();
  showHostAbortButton(false);
...
function injectHostAbortButton() {
  ...
  btn.textContent = "✕ Lobby schließen";
  btn.title = "Lobby schließen und alle Spieler rauswerfen";

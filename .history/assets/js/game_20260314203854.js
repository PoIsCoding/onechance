...
function enterCluePhase() {
  ...
  if (State.isGuesser || State.isTV) {
    if (State.isTV) {
      // TV-Screen: Geheimwort groß anzeigen + live Tipp-Counter starten
...
async function enterRevealPhase() {
  ...
  if (State.isTV) {
    // TV-Spieler sieht die Reveal-Animation auf dem Fernseher!
    // Warte-Screen mit Hinweis dass Mod/Host noch prüft
    document.getElementById("tv-wait-title").textContent = "Gleich geht's los…";
    document.getElementById("tv-wait-sub").textContent =
      "Die Hinweise werden vorbereitet.";
    showScreen("tvWait");
    watchForPhase("guess");
    return;
  }

...
// Statt alle Sounds stumm abzuspielen (hörbar auf iOS!) nutzen wir
// einen leeren AudioContext der beim Beitreten-Button entsperrt wird.
// playSound() wartet bis der Unlock passiert ist.
...
function createAudioContext() {
  ...
  try {
    ...
    log('AudioContext erstellt, state:', audioContext.state);
  } catch (e) {
    log('AudioContext nicht verfügbar:', e.message);
  }
}

// Wird beim Beitreten-Button aufgerufen (echter User-Gesture)
function unlockAudioContext() {
  if (audioUnlocked) return;
  createAudioContext();
  if (!audioContext) return;

  // AudioContext aus suspended-State aufwecken
  if (audioContext.state === 'suspended') {
    audioContext.resume().then(() => {
      audioUnlocked = true;
      log('Audio-Unlock: AudioContext resumed');
    }).catch(e => log('Audio-Unlock fehlgeschlagen:', e.message));
  } else {
    audioUnlocked = true;
    log('Audio-Unlock: AudioContext bereits aktiv');
  }
}
...
// ══════════════════════════════════════════════════════════════
//  STATISTIK
//  Schreibt aggregierte Daten nach Firebase unter /stats/
//  Nur der Host schreibt Spiel-Stats (Runden, Ergebnis, Kategorie)
//  Jeder Client schreibt seinen eigenen User-Eintrag (unique users)
// ══════════════════════════════════════════════════════════════

// Datum im Format YYYY-MM-DD für tagesbasierte Stats
function todayKey() {
...
async function trackGameResult(correct, secretWord, validClueCount) {
  ...
  if (correct) {
    ...
    await statIncrement(`stats/daily/${today}/gamesWon`);
  } else {
    await statIncrement("stats/totals/gamesLost");
...
const State = {
  ...
  _inactivityTimer: null, // setInterval-Handle für Auto-Close nach Inaktivität
};
...
// Wartet auf anonymen Firebase-Login bevor das Spiel startet
...
async function checkHostReconnect(uid) {
  ...
  try {
    ...
    // Spieler-Eintrag wiederherstellen falls durch beforeunload entfernt
    if (!data.players?.[uid]) {
      await State.db
    ...
    // Falls Spiel läuft → in aktuelle Phase einsteigen
    if (data.phase && data.phase !== "lobby") {
      ...
      enterLobbyScreen(); // ruft startCentralListener() selbst auf
    }
    return true;
  } catch (e) {
...
async function checkPlayerReconnect(uid) {
  ...
  try {
    ...
    // Spieler wieder eintragen falls er durch beforeunload entfernt wurde
    if (!players[uid]) {
      const savedName = localStorage.getItem("onechance_name") || "Spieler";
      log("UID nicht in players – trage Spieler wieder ein für Reconnect");
      try {
        ...
        // Frisch lesen
        const snap2 = await State.db.ref(`lobbies/${savedCode}/players`).once("value");
        players[uid] = snap2.val()?.[uid];
        if (!players[uid]) {
          localStorage.removeItem("onechance_player_lobby");
          return false;
        }
      } catch(e) {
        log("Spieler-Wiedereintrag fehlgeschlagen:", e);
        localStorage.removeItem("onechance_player_lobby");
        return false;
      }
    }

    // Lobby-Phase: Spieler zurück in die Lobby bringen
    if (data.phase === "lobby") {
      log("Spieler-Reconnect in Lobby-Phase");
      State.lobbyCode  = savedCode;
...
async function joinOrCreateLobby(name, code, role) {
  ...
  // beforeunload: Spieler nur aus DB entfernen wenn er wirklich die Seite verlässt
  // NICHT beim Reload – damit Reconnect funktioniert.
  // Wir nutzen sessionStorage als Reload-Detektor:
  // - Beim ersten Load: sessionStorage hat keinen Eintrag → neuer Besuch
  // - Beim Reload: sessionStorage hat den Eintrag noch → Reload
  window.addEventListener("beforeunload", () => {
    if (!State.isHost && State.lobbyCode) {
      // Wir können beforeunload nicht zuverlässig zwischen Reload und
      // echtem Verlassen unterscheiden. Daher: Spieler NICHT hier löschen.
      // Stattdessen: onDisconnect in Firebase nutzen (wird nach ~60s ausgelöst).
      // Der Spieler bleibt in players stehen und kann reconnecten.
      log("beforeunload – Spieler bleibt in DB für Reconnect");
    }
  });
}
...
async function setTVPlayer(uid) {
  ...
  if (current === uid) {
    ...
    // QR nur beim TV-Spieler selbst anzeigen – nicht beim Host
    // Der tvUID-Listener in enterLobbyScreen übernimmt das für alle Clients
...
function enterLobbyScreen() {
  ...
  // QR-Code: nur für den TV-Spieler in der Lobby anzeigen
  if (State.isTV && State.tvUID) {
  ...
  addListener(
    ...
    (snap) => {
      ...
      if (State.isTV) generateQR(State.lobbyCode);
  ...
  // Inaktivitäts-Timer starten (nur Host) + Aktivität registrieren
  touchActivity();
...
async function startGame() {
  ...
  startCentralListener();
  touchActivity(); // Aktivität registrieren
}
...
// ── TV: Countdown vor Anzeige des Geheimworts ──
// Zählt von n auf 0, dann wird callback aufgerufen
function startTVCountdown(seconds, callback) {
  const numEl = document.getElementById("tv-countdown-number");
  ...
  const tick = setInterval(() => {
    ...
    if (remaining <= 0) {
      ...
      // Puls-Animation bei jedem Tick
      numEl.classList.remove("tv-countdown-pulse");
      void numEl.offsetWidth; // reflow
      numEl.classList.add("tv-countdown-pulse");
    }
  }, 1000);
}

// ── TV: Tipp-Counter live aktualisieren ──
// Zeigt auf dem TV-Screen wie viele Hinweise bereits eingegangen sind.
function watchClueCounterForTV() {
  addListener(
    State.db.ref(`lobbies/${State.lobbyCode}/players`),
    "value",
    (snap) => {
      const players = snap.val() || {};
      // Tippgeber = aktiv, kein Zuschauer, kein Mod, kein TV-Spieler selbst
      const giverCount = Object.entries(players).filter(
        ([uid, p]) =>
          p.role !== "zuschauer" &&
          uid !== State.modUID &&
          uid !== State.guesserUID,
      ).length;
      // Clues live mitlesen
      State.db
        ...
        .once("value")
        .then((s) => {
          const clueCount = Object.keys(s.val() || {}).length;
          const el = document.getElementById("tv-clue-counter");
          if (el) el.textContent = `${clueCount} / ${giverCount}`;
        });
    },
  );

  // Auch Clue-Änderungen direkt tracken
  addListener(
    State.db.ref(`lobbies/${State.lobbyCode}/clues`),
    "value",
    async (snap) => {
      const clueCount = Object.keys(snap.val() || {}).length;
      const playersSnap = await State.db
        .ref(`lobbies/${State.lobbyCode}/players`)
        .once("value");
      const players = playersSnap.val() || {};
...
async function enterRevealPhase() {
  ...
  // Normaler Rater (nicht TV): Warte-Screen
  if (State.isGuesser && !State.isTV) {
    showScreen("guesserWait");
    document.querySelector("#screen-guesser-wait .wait-title").textContent =
      "Gleich geht's los…";
  ...
  // TV-Spieler: fällt durch → sieht die Reveal-Animation wie alle anderen
  // (isTV ist kein Grund mehr den Screen zu überspringen)
...
function isContainedInSecretWord(clueText, secretWord) {
  ...
  // ── 1. Direkter Teilstring ──
  if (secret.includes(clue) || clue.includes(secret)) return true;

  // ── 2. Wortstamm-Erkennung ──
  // Sucht den längsten gemeinsamen Teilstring (mind. 4 Zeichen).
  // 4 Zeichen damit Verbstämme erkannt werden:
  //   "fahren"  → "fahr"  (4) in "fahrrad"   ✓
  //   "laufen"  → "lauf"  (4) in "laufband"  ✓
  //   "kochen"  → "koch"  (4) in "kochbuch"  ✓
  // Aber kurze unspezifische Silben wie "ist", "ein", "auf" bleiben unter 4.
  const MIN_STEM = 4;

  // Alle Teilstrings von clue mit Länge >= MIN_STEM prüfen
  for (let len = clue.length; len >= MIN_STEM; len--) {
    for (let start = 0; start <= clue.length - len; start++) {
      const stem = clue.substring(start, start + len);
      if (secret.includes(stem)) {
        log(`Stamm-Treffer: "${clue}" ↔ "${secret}" via Stamm "${stem}"`);
        return true;
      }
    }
  }
...
async function enterGuessPhase() {
  ...
  if (State.isGuesser || State.isTV) {
    if (State.isTV) {
      // TV-Screen: Hinweise anzeigen, kein Eingabefeld
      const tvList = document.getElementById("tv-clue-list");
      tvList.innerHTML = "";
  ...
  // TV-Modus: Host/Mod sehen Verdikt-Buttons direkt in der Guess-Phase
  if (State.tvUID && (State.isHost || State.isMod)) {
    log("TV-Modus Guess – Host/Mod zeigt Verdikt-Screen");
    showTVVerdictScreen();
    return;
  }
...
// TV-Modus: Host/Mod sieht Richtig/Falsch Buttons während
// der TV-Spieler die Hinweise auf dem Fernseher sieht
function showTVVerdictScreen() {
  log("TV-Verdikt-Screen anzeigen");
  document.getElementById("result-icon").textContent = "🤔";
  document.getElementById("result-title").textContent = "Hat er es erraten?";
  ...
  wordEl.innerHTML = `Gesuchtes Wort: <strong>${State.secretWord}</strong>`;
  document.getElementById("mod-verdict-btns").classList.remove("hidden");
...
function renderResultScreen(guess, correct) {
  ...
  // Bei richtiger Antwort steht das Wort bereits im Guess – nicht nochmal anzeigen
  const wordEl = document.getElementById("result-word");
  if (correct) {
    wordEl.innerHTML = "";
    ...
    wordEl.style.display = "";
    wordEl.innerHTML = `Das gesuchte Wort war: <strong>${State.secretWord}</strong>`;
  }
...
async function touchActivity() {
  if (!State.lobbyCode || !State.db) return;
  await State.db.ref(`lobbies/${State.lobbyCode}/lastActivity`).set(Date.now());
}

// Startet den Inaktivitäts-Timer (nur beim Host)
// Prüft alle 60 Sek ob die letzte Aktivität > 30 min zurückliegt
function startInactivityTimer() {
  stopInactivityTimer(); // vorher stoppen falls läuft
  ...
  State._inactivityTimer = setInterval(async () => {
    ...
    const snap = await State.db
      ...
      .once("value");
    const last = snap.val();
    if (!last) return; // noch kein Timestamp → warten

    const elapsed = Date.now() - last;
    log(`Inaktivität: ${Math.round(elapsed / 1000 / 60)} min`);

    if (elapsed >= INACTIVITY_MS) {
      log("Lobby inaktiv – automatisch schließen");
      stopInactivityTimer();
      removeCentralListener();
      removeAllListeners();
...
function stopInactivityTimer() {
  if (State._inactivityTimer) {
    clearInterval(State._inactivityTimer);
    State._inactivityTimer = null;
    log("Inaktivitäts-Timer gestoppt");
  }
}
...
document.getElementById("btn-join").addEventListener("click", async () => {
  // Audio-Kontext beim echten User-Tap entsperren (iOS-Anforderung)
  unlockAudioContext();
...
(async function init() {
  ...
  // Auf anonymen Login warten bevor irgendwas mit der DB passiert
  await waitForAuth();
